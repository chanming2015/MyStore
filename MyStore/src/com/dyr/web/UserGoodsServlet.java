...
public class UserGoodsServlet extends HttpServlet {
	...
	private void showGoodsInfo(HttpServletRequest request,
		...
		try {
			...
			pageBean.setRows(10);
			List<Discuss> discussList = discussService.getPassDiscussInfoBydGId(
					goodsId,pageBean.getPage(), pageBean.getRows());
			
			pageBean.setRowsCount(discussService.getPassCountByDsh(goodsId));
			pageBean.setPagesCount();
			pageBean.setDataList(discussList);
	 ...
	 * @param dateList
	 */
	private void setPageBean(int rowsCount, List<?> dateList,PageBean pageBean) {
		
		pageBean.setRowsCount(rowsCount);
		pageBean.setPagesCount();
		pageBean.setDataList(dateList);
	}
