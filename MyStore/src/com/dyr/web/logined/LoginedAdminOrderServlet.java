...
public class LoginedAdminOrderServlet extends HttpServlet {
	...
	private void showList(HttpServletRequest request,
		...
		if(keyword!=null && !"".equals(keyword)){
			//按关键字模糊查询
			rowsCount=orderUserService.getOrderCountByKeyWord(keyword);
			pageBean.setRowsCount(rowsCount);
			pageBean.setPagesCount();
			orderUserList=orderUserService.getOrderListByKeyWord(keyword,pageBean.getPage(),pageBean.getRows());
		}else {
			rowsCount=orderUserService.getOrderCount();
			pageBean.setRowsCount(rowsCount);
			pageBean.setPagesCount();
			//分页查询订单
			orderUserList=orderUserService.getOrdersByPage(pageBean.getPage(),pageBean.getRows());
		}
